/*
 * Sideous Umbra - simple file-based preset storage, entirely UI-side (no DPF
 ...
 * array). One plain-text ".supreset" file per preset, one "symbol=value"
 ...
 *
 * One difference from sideous-noise's version this was ported from: Sideous
 * Umbra has kParamOutLevel, an output-only meter param (drives the cat
 * mascot's eye brightness, see paintUmbraEyes() in ui/UIPainter.hpp) that
 * isn't a real user control and has no place in a saved preset - it's
 * explicitly skipped in the savePreset()/loadPreset() loops below.
 */
...
inline std::string presetsDirectory()
{
    ...
    return appData ? std::string(appData) + "\\sideous-umbra\\presets" : ".\\presets";
#elif defined(__APPLE__)
    const char* home = std::getenv("HOME");
    return home ? std::string(home) + "/Library/Application Support/sideous-umbra/presets" : "./presets";
#else
    if (const char* xdg = std::getenv("XDG_DATA_HOME"))
        return std::string(xdg) + "/sideous-umbra/presets";
    const char* home = std::getenv("HOME");
    return home ? std::string(home) + "/.local/share/sideous-umbra/presets" : "./presets";
#endif
}
...
// sorted (alphabetical) list of preset display names - the ".supreset"
...
inline std::vector<std::string> listPresets()
{
    ...
    static constexpr const char* kExt = ".supreset";
...
inline bool savePreset(const std::string& name, const float* values)
{
    ...
    std::ofstream out(presetsDirectory() + "/" + name + ".supreset");
    if (!out.is_open())
        return false;
    ...
    for (uint32_t i = 0; i < kParamCount; ++i)
    {
        ...
        // it must never be written to a preset file
        if (i == kParamOutLevel)
            continue;
...
{
    std::ifstream in(presetsDirectory() + "/" + name + ".supreset");
    if (!in.is_open())
        return false;
    ...
    while (std::getline(in, line))
    {
        ...
        for (uint32_t i = 0; i < kParamCount; ++i)
        {
            ...
            // parameter - never read it back from a preset file even if an
            // old/hand-edited file happens to contain "eye_level=..."
            if (i == kParamOutLevel)
                continue;
...
{
    return std::remove((presetsDirectory() + "/" + name + ".supreset").c_str()) == 0;
}
